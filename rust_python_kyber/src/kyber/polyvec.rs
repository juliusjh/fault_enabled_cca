...
impl Polyvec {
    ...
    fn __add__(&self, rhs: Polyvec) -> PyResult<Polyvec> {
        let mut res = Polyvec::new();
        polyvec_add(&mut res, &self, &rhs);
        Ok(res)
    }

    fn __sub__(&self, rhs: Polyvec) -> PyResult<Polyvec> {
        let mut res = Polyvec::new();
        for (mut resi, lhsi, rhsi) in izip!(res.vec.iter_mut(), self.vec.iter(), rhs.vec.iter()) {
