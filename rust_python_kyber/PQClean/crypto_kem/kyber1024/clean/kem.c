...
int PQCLEAN_KYBER1024_CLEAN_crypto_kem_keypair_manip(unsigned char pk[KYBER_PUBLICKEYBYTES],
        unsigned char sk[KYBER_SECRETKEYBYTES], polyvec* ecp) {
    size_t i;
    PQCLEAN_KYBER1024_CLEAN_indcpa_keypair_manip(pk, sk, ecp);
    for (i = 0; i < KYBER_INDCPA_PUBLICKEYBYTES; i++) {
        sk[i + KYBER_INDCPA_SECRETKEYBYTES] = pk[i];
    }
    hash_h(sk + KYBER_SECRETKEYBYTES - 2 * KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES);
...
* Returns 0 (success)
**************************************************/
int PQCLEAN_KYBER1024_CLEAN_crypto_kem_enc_manip(unsigned char ct[KYBER_CIPHERTEXTBYTES],
    ...
    PQCLEAN_KYBER1024_CLEAN_indcpa_enc_manip(ct, buf, pk, kr + KYBER_SYMBYTES, r, e1, e2);
...
int PQCLEAN_KYBER1024_CLEAN_crypto_kem_dec_glitch(unsigned char ss[KYBER_SSBYTES],
        const unsigned char ct[KYBER_CIPHERTEXTBYTES],
        const unsigned char sk[KYBER_SECRETKEYBYTES],
    ...
    fail = PQCLEAN_KYBER1024_CLEAN_verify(ct_manip, cmp, KYBER_CIPHERTEXTBYTES);
