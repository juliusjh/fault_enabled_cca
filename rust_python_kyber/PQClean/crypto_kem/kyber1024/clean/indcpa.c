...
void PQCLEAN_KYBER1024_CLEAN_indcpa_keypair_manip(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
        uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES], polyvec* ecp) {
    unsigned int i;
    uint8_t buf[2 * KYBER_SYMBYTES];
    const uint8_t *publicseed = buf;
    const uint8_t *noiseseed = buf + KYBER_SYMBYTES;
    ...
    memcpy(ecp, &e, sizeof(polyvec));
    PQCLEAN_KYBER1024_CLEAN_polyvec_ntt(&skpv);
    PQCLEAN_KYBER1024_CLEAN_polyvec_ntt(&e);

    // matrix-vector multiplication
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER1024_CLEAN_polyvec_basemul_acc_montgomery(&pkpv.vec[i], &a[i], &skpv); //*R^-1
        PQCLEAN_KYBER1024_CLEAN_poly_tomont(&pkpv.vec[i]); //*R
    }
...
void PQCLEAN_KYBER1024_CLEAN_indcpa_enc_manip(uint8_t c[KYBER_INDCPA_BYTES],
                                        const uint8_t m[KYBER_INDCPA_MSGBYTES],
                                        const uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
                                        const uint8_t coins[KYBER_SYMBYTES],
                                        polyvec* r,
    ...
    memcpy(r, &sp, sizeof(sp));

    PQCLEAN_KYBER1024_CLEAN_polyvec_ntt(&sp);

    memcpy(e2, &epp, sizeof(epp));
    memcpy(e1, &ep, sizeof(ep));
...
void PQCLEAN_KYBER1024_CLEAN_indcpa_dec(uint8_t m[KYBER_INDCPA_MSGBYTES],
    ...
    /*
    printf("Error term incl msg pol: [");
    for (int i = 0; i < 256; i++) {
        printf("%i, ", mp.coeffs[i]);
    }
    printf("\n");
    */
